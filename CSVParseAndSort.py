#Customer.io Application Script - Aaron Kratz
import csv as csv
...
#Sorts pulledRows in ascending order by date and returns a string containing the ordered values from the words column
def generatePhrase(pulledRows):
    selectRows(rows)
    resultPhrase = " "
    
    for x in range(1, len(pulledRows)):             #Insertion sorts list of rows from .csv by values in start date column
